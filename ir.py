...
# fn. for generating virtual registers
def genTemp():
    r = {'t' : 0}
    def new():
        r['t'] += 1
...
class Var(IRNode):
    ...
    def lower(self):
        vr = Temp()
        s = LoadStat(parent=self.parent, symbol=vr, symtab=self.symtab)
        return self.parent.replace(self, vr)
...
class BinExpr(Expr):
    ...
    def lower(self):
        lop, rop = self.getOperands()
        f = False

        if hasattr(lop, 'lower'):
            if lop.parent == None:
                lop.parent = self
            f = lop.lower()

        if hasattr(rop, 'lower'):
            if rop.parent == None:
                rop.parent = self
            f |= rop.lower()
...
class ForStat(Stat):
    ...
    def collect_uses(self):
        self.init.collect_uses() + self.step.collect_uses()
...
class AssignStat(Stat):
    ...
    def lower(self):
        ...
        if hasattr(self.expr, 'lower'):
            self.expr.lower()

        s = StoreStat(parent=self.parent, dst=vr, src=self.expr, symtab=self.symtab)

        return self.parent.replace(self, s)
...
class StoreStat(Stat):
    def __init__(self, parent=None, dst=None, src=None, symtab=None):
        self.parent=parent
        self.symbol=dst
        self.src=src
        self.symtab=symtab
    ...
    def lower(self):
        if hasattr(self.src, 'lower'):
            if self.src.parent == None:
                self.src.parent = self
